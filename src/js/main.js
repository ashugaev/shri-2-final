...
(function() {
	...
	while (rotation < 359) {
		...
		circle.insertAdjacentHTML(
			'beforeEnd',
			`<div class="circle-regulator__dash" style="transform:rotate( ${rotation}deg);   ${rotation < rotated
				? 'border-color:' + color
				: ''};"></div>`
		);
	...
	const changeTemperature = function(posX, posY) {
		...
		var mouseTop = posY - (circle.getBoundingClientRect().top + circle.clientHeight / 2) + 0.1;
		...
		for (let i = 0; dashes.length > i; i++) {
			i < dashesToChanches ? (dashes[i].style.borderColor = color) : (dashes[i].style.borderColor = '#333333');

			// На последней итерации меняем угол наклона стрелки
			if (i === dashes.length - 1) {
				document.querySelector('.circle-regulator__arrow').style.transform = `rotate(${absoluteAngle}deg)`;
...
touchRegulator('.stripe-regulator__gradient-stripe.stripe-regulator__gradient-stripe_yellow', '#circle-orange');
touchRegulator(
